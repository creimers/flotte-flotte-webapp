...
export default function Bookings() {
  ...
  if (authState === AuthStatus.authenticated) {
    return (
      ...
      >
        <div className="flex flex-col space-y-2 sm:flex-row sm:space-y-0 justify-between mb-4 sm:items-center">
        ...
        {!loading &&
          ...
          data?.bookingCount > 0 && (
            <div className="prose">
              <p>Du hast keine anstehenden Buchungen.</p>
              <p className="text-gray-400">
                Bisherige Buchungen: {data.bookingCount}
              </p>
            </div>
          )}
        {!loading &&
          data?.bookings?.edges &&
          data?.bookings?.edges.length > 0 && (
              ...
              <tbody>
                {data?.bookings?.edges.map((edge, i) => (
                  <tr
                    ...
                    onClick={() => {
                      router.push(`/bookings/${edge?.node?.uuid}`);
                    }}
                  >
